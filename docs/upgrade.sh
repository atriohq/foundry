...
echo "This will upgrade Foundry"
echo "Make sure you have a backup of the database and the Foundry directory."
...
# Prompt for Foundry path (default: /var/www/foundry)
read -p "Enter path to Foundry [default: /var/www/foundry]: " foundry_path
foundry_path=${foundry_path:-/var/www/foundry}
echo "Using Foundry path: $foundry_path"
...
tar -czf "$backup_dir/foundry_backup_${backup_ts}.tar.gz" \
    -C "$(dirname "$foundry_path")" "$(basename "$foundry_path")"

# Database credentials from .env in Foundry path
...
# Clear cache
echo "Clearing cache..."
php "$foundry_path/bin/clear-cache.php"
...
git clone https://github.com/atriohq/foundry /tmp/foundry
...
copy_files "/tmp/foundry" "$foundry_path"
...
if [[ $? -eq 0 ]]; then
    echo "Services started successfully. Deleting /tmp/foundry..."
    rm -rf /tmp/foundry
else
    echo "There was an issue starting the services. /tmp/foundry will not be deleted."
fi
