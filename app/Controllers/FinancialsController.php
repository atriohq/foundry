...
class FinancialsController extends Controller
{
    ...
    public function payInvoice(Request $request, Response $response, $args)
    {
        ...
        $_SESSION['pending_invoice_amount'] = number_format($totalAmount, 2, '.', '');
        $_SESSION['pending_invoice_id'] = (int)$invoice_details['id'];
    ...
    public function paymentWebhook(Request $request, Response $response, string $gateway): Response {
        ...
        try {
            ...
            error_log(
                "Payment webhook processing failed for {$gateway}: "
                . $e->getMessage()
            );
    ...
    private function paymentContextFromRequest(Request $request, string $gateway): array
    {
        ...
        $db = $this->container->get('db');
        $user = $db->selectRow(
            'SELECT id, currency, email, username FROM users WHERE id = ? LIMIT 1',
            [$userId]
        );
        if (!$user) {
        ...
        $currency = strtoupper(trim((string)$user['currency']));
        ...
        if ($postedAmount === '' && isset(
            $_SESSION['pending_invoice_amount'],
            $_SESSION['pending_invoice_id']
        )) {
            $pendingInvoice = $_SESSION['pending_invoice_id'];
            ...
            if (!$invoice || !in_array(
                ...
                throw new \InvalidArgumentException('The invoice is no longer payable.');
            }

            $type = 'invoice';
            $invoiceId = (int)$invoice['id'];
            $amount = $this->normalizeAmount($_SESSION['pending_invoice_amount']);
    ...
    private function decodePaymentContext(string $gateway, string $token): array
    {
        ...
        $userId = $this->base36Decode($parts[1]);
    ...
    private function settlePayment(array $payment): void
    {
        ...
        try {
            ...
            if (strtoupper((string)$user['currency']) !== $currency) {
            ...
            $db->insert('transactions', [
                'user_id' => $userId,
                'related_entity_type' => $paymentType,
                'related_entity_id' => $invoiceId,
            ...
            if ($paymentType === 'invoice') {
                $updated = $db->exec(
                    ...
                    ['paid', $now, $invoiceId, $userId, 'unpaid', 'overdue']
                );
                if ($updated !== 1) {
